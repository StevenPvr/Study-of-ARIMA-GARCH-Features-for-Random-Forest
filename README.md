...
Ce projet évalue l'impact de l'ajout de features issues d'un modèle **ARIMA-GARCH** sur les performances de prévision d'un modèle **Random Forest** pour les rendements du S&P 500.

### Objectif Principal

Tester l'hypothèse selon laquelle les modèles ARIMA-GARCH capturent des patterns temporels et de volatilité conditionnelle qui peuvent enrichir un modèle Random Forest et améliorer ses prévisions de rendements.
...
4. **Pipeline Random Forest** : Modèle ML avec features ARIMA-GARCH
...
- `scikit-learn` : Random Forest et métriques
...
│   └── rf_dataset_*.csv     # Datasets pour Random Forest
├── src/
...
│   │   ├── garch_eval/
│   │   └── rolling_garch/
│   ├── random_forest/        # Pipeline Random Forest
│   │   ├── data_preparation/
│   │   ├── optimisation/
│   │   ├── training/
│   │   ├── eval/
...
4. Génération des features pour Random Forest
5. Optimisation et entraînement Random Forest
...
- **Random Forest** : Optimisation bayésienne avec Optuna, walk-forward CV
...
- **Random Forest** : Corrélations, SHAP values, importance des features
...
1. **H1** : Les features ARIMA-GARCH améliorent les prévisions du Random Forest
