...
from src.garch.garch_params.estimation_batch import (
    estimate_all_distributions,
    run_batch_estimation_and_save,
)
...
def test_save_model_and_metadata(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert metadata["o"] == 1
    assert metadata["p"] == 1
    assert metadata["distribution"] == "normal"
...
def test_train_egarch_from_dataset(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from src.garch.garch_params import estimation_batch

    monkeypatch.setattr(estimation_batch, "GARCH_ESTIMATION_FILE", est_path)

    # Mock batch estimation to avoid actual estimation
    with patch("src.garch.garch_params.estimation_batch.run_batch_estimation_and_save"):
    ...
    assert result["dist"] in ["normal", "student", "skewt"]
    assert result["n_train"] > 0
    assert "params" in result
    assert "hyperparams" in result
    assert result["hyperparams"]["distribution"] == "normal"  # Check hyperparams not result dist
...
# ============================================================================
# Tests for garch_params.estimation_batch.py
# ============================================================================


def test_estimate_all_distributions() -> None:
    """Test estimating all distributions."""
    resid = _simulate_garch11(300, 0.02, 0.05, 0.9, seed=42)

    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", category=RuntimeWarning)
        warnings.filterwarnings("ignore", category=UserWarning)
        results = estimate_all_distributions(resid, o=1, p=1)

    assert isinstance(results, dict)
    assert len(results) > 0
    # Check that we have at least one distribution result (with egarch_ prefix)
    assert any(dist in results for dist in ["egarch_normal", "egarch_student", "egarch_skewt"])
...
def test_run_batch_estimation_and_save(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Patch GARCH_ESTIMATION_FILE in estimation_batch module where it's used
    from src.garch.garch_params import estimation_batch
    ...
    df = _create_test_dataframe(300)
    df.loc[df["split"] == "train", "sarima_resid"] = _simulate_garch11(
        len(df[df["split"] == "train"]), 0.02, 0.05, 0.9, seed=42
    )
    ...
    with open(est_path) as f:
        results = json.load(f)
    # Check structure from new implementation
    assert "egarch_normal" in results
    assert "egarch_student" in results
    assert "egarch_skewt" in results
...
def test_save_estimation_results(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fits = {
        "normal": {
            "params": {"omega": 0.02, "alpha": 0.05, "gamma": 0.0, "beta": 0.9},
            "converged": True,
...
def test_egarch_forecaster_init_invalid_order() -> None:
    ...
    with pytest.raises(ValueError, match="ARCH order"):
        EGARCHForecaster(o=3, p=1, dist="normal")

    with pytest.raises(ValueError, match="GARCH order"):
        EGARCHForecaster(o=1, p=3, dist="normal")
...
def test_egarch_forecaster_init_invalid_window_type() -> None:
    ...
    with pytest.raises(ValueError, match="window_type"):
        EGARCHForecaster(o=1, p=1, dist="normal", window_type="invalid")
...
def test_egarch_forecaster_init_rolling_window_no_size() -> None:
    ...
    with pytest.raises(ValueError, match="window_size required"):
        EGARCHForecaster(o=1, p=1, dist="normal", window_type="rolling", window_size=None)
