...
from src.garch.garch_params.refit.windows import ExpandingWindow, RollingWindow
...
class TestForecastTemporalCausality:
    ...
    def test_forecast_expanding_uses_correct_history_length(self):
        ...
        with patch.object(
            forecaster, "_compute_one_step_forecast", wraps=forecaster._compute_one_step_forecast
        ) as mock_forecast:
...
class TestTrainTestSeparation:
    ...
    def test_forecaster_train_test_continuity(self):
        ...
        # Créer données TRAIN et TEST
        resid_train = np.random.randn(70)
        resid_test = np.random.randn(30)
