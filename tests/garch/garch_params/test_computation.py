...
def test_validate_series_insufficient_observations() -> None:
    """Test validate_residuals raises ValueError for insufficient observations."""
    e = np.array([1.0, 2.0])  # Too few observations
    with pytest.raises(ValueError, match="at least"):
        validate_residuals(e)


def test_validate_series_2d_array() -> None:
    """Test validate_residuals converts 2D array to 1D."""
    rng = np.random.default_rng(42)
    e_2d = rng.normal(0.0, 0.01, size=(100, 1))
    result = validate_residuals(e_2d)
...
def test_egarch_kappa_student_missing_nu() -> None:
    """Test egarch_kappa uses default nu for student distribution."""
    # Should not raise an error, should use default nu
    kappa = egarch_kappa("student", None, None)
