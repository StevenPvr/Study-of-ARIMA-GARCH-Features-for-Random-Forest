...
class TestSplitTickersTrainTest:
    ...
    def test_split_tickers_train_test_default(
        ...
        # NOTE: After anti-leakage contaminated observation removal,
        # the observed ratio will be higher than target (less test data)
        # Allow for wider tolerance to account for this
        assert abs(train_ratio_observed - 0.8) < 0.15
    ...
    def test_split_tickers_train_test_single_ticker_too_small(self, tmp_path: Path) -> None:
        ...
        output_file = tmp_path / "ticker_split.parquet"
        # After computing log_returns, single observation is dropped (first obs has NaN)
        # This results in empty DataFrame error
        with pytest.raises(ValueError, match="(No ticker data could be split|Input ticker data DataFrame is empty)"):
            split_tickers_train_test(
