...
def test_determine_stationarity() -> None:
    """Test _determine_stationarity logic directly."""
    from src.arima.stationnarity_check import _determine_stationarity

    # Test case 1: Stationary (ADF rejects unit root, KPSS accepts stationarity)
    adf_stationary = {
        "statistic": -3.5,
        "p_value": 0.01,
        "lags": 1,
        "nobs": 100,
        "critical_values": {"1%": -3.43},
    }  # p < 0.05
    kpss_stationary = {
        "statistic": 0.5,
        "p_value": 0.10,
        "lags": 1,
        "nobs": 100,
        "critical_values": {"1%": 0.739},
    }  # p > 0.05
    result = _determine_stationarity(
        cast(StationarityTestResult, adf_stationary),
        cast(StationarityTestResult, kpss_stationary),
        0.05,
    )
    assert result is True

    # Test case 2: Non-stationary (ADF fails to reject unit root)
    adf_non_stationary = {
        "statistic": -1.5,
        "p_value": 0.10,
        "lags": 1,
    ...
    result = _determine_stationarity(
        cast(StationarityTestResult, adf_non_stationary),
        cast(StationarityTestResult, kpss_stationary),
        0.05,
    )
    assert result is False

    # Test case 3: Non-stationary (KPSS rejects stationarity)
    adf_stationary = {
        "statistic": -3.5,
        "p_value": 0.01,
        "lags": 1,
    ...
    result = _determine_stationarity(
        cast(StationarityTestResult, adf_stationary),
        cast(StationarityTestResult, kpss_non_stationary),
        0.05,
    )
    assert result is False

    # Test case 4: KPSS returns NaN (should be considered as accepting stationarity)
    adf_stationary = {
        "statistic": -3.5,
        "p_value": 0.01,
        "lags": 1,
    ...
    result = _determine_stationarity(
        cast(StationarityTestResult, adf_stationary), cast(StationarityTestResult, kpss_nan), 0.05
    )
    assert result is True


def test_extract_value_from_zivot_andrews_result() -> None:
    """Test _extract_value_from_zivot_andrews_result with different scenarios."""
    from src.arima.stationnarity_check.stationnarity_check import (
        _extract_value_from_zivot_andrews_result,
    )

    # Test extraction of int value (lags) - has dict at third position
    values_with_dict = [1.0, 0.05, {"1%": -3.5}, 5, 100]  # dict at index 2
    result = _extract_value_from_zivot_andrews_result(
        values_with_dict, index_with_dict=3, index_without_dict=2, expected_type=int
    )
    assert result == 5

    # Test extraction of int value - no dict at third position
    values_no_dict = [1.0, 0.05, 2, 3, 100]  # no dict at index 2, lags at index 2
    result = _extract_value_from_zivot_andrews_result(
        values_no_dict, index_with_dict=3, index_without_dict=2, expected_type=int
    )
    assert result == 2

    # Test extraction of dict value (critical values)
    result = _extract_value_from_zivot_andrews_result(
        values_with_dict, index_with_dict=2, index_without_dict=4, expected_type=dict
    )
    assert result == {"1%": -3.5}

    # Test fallback for nobs when series_size provided
    result = _extract_value_from_zivot_andrews_result(
        values_no_dict, index_with_dict=5, index_without_dict=5, expected_type=int, series_size=150
    )
    assert result == 150

    # Test None fallback when no series_size
    result = _extract_value_from_zivot_andrews_result(
        values_no_dict,
        index_with_dict=5,
        index_without_dict=5,
        expected_type=int,
        fallback_value=None,
    )
