...
class TestOptimizeSarimaModels:
    ...
    def _create_mock_fitted_model() -> MagicMock:
        """Create a mock fitted SARIMA model."""
        import numpy as _np
        ...
        def mock_run_optuna_side_effect(
            ...
            for i in range(2):  # Return 2 results
                result = {
                    ...
                    "aic": -100.0 - i,
                    "bic": -95.0 - i,
                    "composite_score": -100.0 - i,  # Same as AIC for simple test case
                    "lb_stat": 10.0 + i,
                    "lb_pvalue": 0.1 + i * 0.01,
                }
    ...
    def test_optimize_sarima_models_invalid_inputs(self) -> None:
        ...
        with pytest.raises(ValueError, match="train_series is empty"):
            optimize_sarima_models(
    ...
    def test_optimize_sarima_models_partial_convergence(
        ...
        results_df, best_aic, best_bic = optimize_sarima_models(
            ...
            test_series,
            n_trials=10,
            backtest_cfg={"n_splits": 5, "test_size": 20},
            out_dir=None,
        )
    ...
    # test_optimize_sarima_models_bic_failure removed - BIC optimization no longer exists

    @patch("src.arima.optimisation_arima.results_processing.save_results")
    @patch("src.arima.optimisation_arima.optimisation_arima._run_optuna")
    def test_optimize_sarima_models_random_state(
        self,
        mock_run_optuna: MagicMock,
        mock_save: MagicMock,
    ) -> None:
        ...
        results_df = pd.DataFrame(
            {
                ...
                "composite_score": [100.0, 95.0, 90.0],  # Same as AIC for this test
                "backtest_rmse_mean": [0.3, 0.2, 0.25],
                "lb_reject_5pct": [False, False, True],
            }
        )
        best_aic, best_bic = pick_best(results_df)
        # Should select model with lowest composite_score (index 2, composite_score=90.0)
...
class TestWalkForwardBacktest:
    ...
    def test_strict_blockwise_refit_and_no_leakage(self) -> None:
        ...
        with patch("src.arima.optimisation_arima.model_evaluation._fit_sarima", new=_mock_fit):
            test_params = SarimaParams(
                p=0, d=0, q=0, P=0, D=0, Q=0, s=1, trend="n", refit_every=refit_every
            )
        ...
        # Sanity: total predicted points equals n_splits * test_size (implicit via forecast calls)
        # 6 forecasts were made with horizons [2,2,2,2,2,2] totaling 12 steps; matches 2*6


class TestLjungBoxFiltering:
    """Tests for Ljung–Box soft-penalty in optimization objective."""

    def test_all_models_fail_ljungbox(self) -> None:
        """All models fail Ljung–Box → finite scores with penalty applied.

        Verifies that:
        - No trial is killed; each has a finite penalized score
        - Penalized score equals AIC + λ * max(0, c - p_LB)
        - Study best value is the minimum of penalized scores
        """
        import numpy as np
        import optuna
        ...
        from src.arima.optimisation_arima.objective import (
            NormalizationStatsCallback,
            objective_aic,
        )

        # Create synthetic series (doesn't matter much as we'll mock the evaluation)
        train = pd.Series(np.random.randn(100))

        # Mock evaluate_param_combination to always return failing Ljung-Box
        def mock_evaluate_failing_lb(*args: Any, **kwargs: Any) -> Dict[str, Any]:
            """Mock that always returns models with significant autocorrelation."""
            return {
                "aic": np.random.uniform(-100, -50),  # Random but reasonable AIC
                "bic": np.random.uniform(-90, -40),
                "lb_stat": 50.0,  # High statistic
                "lb_pvalue": 0.001,  # FAIL: p < 0.05 (significant autocorrelation)
            }

        with patch(
            "src.arima.optimisation_arima.objective.evaluate_param_combination",
            side_effect=mock_evaluate_failing_lb,
        ):
            # Create study
            study = optuna.create_study(direction="minimize")
            stats_callback = NormalizationStatsCallback(criterion="aic")

            # Run optimization with 5 trials
            n_trials = 5
            study.optimize(
                lambda t: objective_aic(t, train, backtest_cfg=None, stats_callback=stats_callback),
                n_trials=n_trials,
                callbacks=[lambda study, trial: stats_callback(study, trial)],
            )

            # Verify all trials are finite and penalized
            assert len(study.trials) == n_trials
            for trial in study.trials:
                assert trial.value is not None and np.isfinite(trial.value)

                # Verify LB p-value was stored
                assert "lb_pvalue" in trial.user_attrs
                pval = float(trial.user_attrs["lb_pvalue"])
                assert pval < SARIMA_LJUNGBOX_P_VALUE_THRESHOLD

                # Expected penalized objective: AIC + λ * (c - p)
                aic = float(trial.user_attrs["aic"])  # set by objective
                expected = aic + SARIMA_LJUNGBOX_PENALTY_WEIGHT * (
                    SARIMA_LJUNGBOX_P_VALUE_THRESHOLD - pval
                )
                assert trial.value == pytest.approx(expected, rel=1e-9, abs=1e-9)

            # Best value should be finite and equal to min penalized value
            trial_values = [t.value for t in study.trials if t.value is not None]
            best = min(trial_values)
            assert np.isfinite(study.best_value)
            assert study.best_value == pytest.approx(best, rel=1e-12)

    def test_partial_ljungbox_filtering(self) -> None:
        """Alternating pass/fail Ljung–Box → penalty applied, not hard rejection.

        Verifies that:
        - Failing trials receive a positive penalty, passing trials do not
        - Best value matches the minimum of penalized scores (may be a failing LB model
          if its AIC advantage dominates the penalty)
        """
        import numpy as np
        import optuna
        from src.constants import (
        ...
        # Create mock that alternates between passing and failing LB test
        call_count = 0

        def mock_evaluate_alternating_lb(*args: Any, **kwargs: Any) -> Dict[str, Any]:
            """Mock that alternates between passing and failing LB test."""
            nonlocal call_count
            call_count += 1

            if call_count % 2 == 0:
                # Even trials: PASS LB test
                return {
                    "aic": -100.0 + call_count,  # Slightly different AIC values
                    "bic": -90.0 + call_count,
                    "composite_score": -100.0 + call_count,
                    "lb_stat": 5.0,
                    "lb_pvalue": 0.8,  # PASS: p > 0.05
                }
            else:
                # Odd trials: FAIL LB test (but have better AIC)
                return {
                    "aic": -200.0,  # Much better AIC, but should be rejected
                    "bic": -190.0,
                    "composite_score": -200.0,
                    "lb_stat": 50.0,
                    "lb_pvalue": 0.001,  # FAIL: p < 0.05
                }

        with patch(
            "src.arima.optimisation_arima.objective.evaluate_param_combination",
            side_effect=mock_evaluate_alternating_lb,
        ):
            study = optuna.create_study(direction="minimize")
            stats_callback = NormalizationStatsCallback(criterion="aic")

            # Run 6 trials (3 pass, 3 fail)
            n_trials = 6
            study.optimize(
                lambda t: objective_aic(t, train, backtest_cfg=None, stats_callback=stats_callback),
                n_trials=n_trials,
            ...
            # Verify penalty behavior per trial
            penalized_values: list[float] = []
            for t in study.trials:
                pval = float(t.user_attrs["lb_pvalue"])  # stored by objective
                aic = float(t.user_attrs["aic"])  # stored by objective
                if pval < SARIMA_LJUNGBOX_P_VALUE_THRESHOLD:
                    expected = aic + SARIMA_LJUNGBOX_PENALTY_WEIGHT * (
                        SARIMA_LJUNGBOX_P_VALUE_THRESHOLD - pval
                    )
                else:
                    expected = aic  # no penalty for passing LB
                penalized_values.append(expected)
                assert t.value == pytest.approx(expected, rel=1e-9, abs=1e-9)

            # Best equals min penalized value (may come from fail LB due to strong AIC)
            assert study.best_value == pytest.approx(min(penalized_values), rel=1e-12)
