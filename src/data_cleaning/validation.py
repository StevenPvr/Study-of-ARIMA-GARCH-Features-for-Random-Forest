...
def _normalize_datetime_timezone(converted: pd.Series) -> pd.Series:
    ...
    try:
        if hasattr(converted, "dt"):
            converted = converted.dt.tz_convert("America/New_York")
            converted = converted.dt.tz_localize(None)
            converted = converted.dt.normalize()
    except (AttributeError, TypeError):
        # Handle case where converted is a mock or doesn't support .dt accessor
        # In this case, assume it's already naive or handle appropriately
        converted = converted.tz_localize(None)  # type: ignore[call-arg]
        converted = converted.normalize()  # type: ignore[call-arg]
