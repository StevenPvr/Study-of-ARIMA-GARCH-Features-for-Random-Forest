...
def format_dates_to_string(
    ...
    # Convert to pandas DatetimeIndex (always has strftime method)
    # Suppress pandas datetime parsing warnings for invalid inputs
    import warnings

    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="Could not infer format", category=UserWarning)
        datetime_index = pd.DatetimeIndex(pd.to_datetime(dates, errors="raise"))

    # strftime on DatetimeIndex returns Index of strings
    formatted_index = datetime_index.strftime(date_format)

    # Convert Index to Series for consistent return type
