...
1. Data pipeline (fetching → cleaning → conversion → preparation)
2. ARIMA/SARIMA pipeline (
   data visualization → stationarity check → optimization → training → evaluation
)
3. GARCH pipeline (
   visualization → numerical tests → structure → params → diagnostic
   → training → eval → benchmark
)
4. LightGBM pipeline (
...
def find_all_main_files() -> list[Path]:
    ...
    main_files = [
        ...
        for f in main_files
        if f != Path(__file__)
        and "SP_Forecasting" not in str(f)
        and "site-packages" not in str(f)
        and "__pycache__" not in str(f)
    ]

    # Also include garch/benchmark/main_vol_backtest.py (not named main.py)
    benchmark_file = src_dir / "garch" / "benchmark" / "main_vol_backtest.py"
    if benchmark_file.exists():
        main_files.append(benchmark_file)
    ...
    def get_priority(path: Path) -> tuple[int, str]:
        ...
        # SARIMA pipeline second (priority 2)
        # Data visualization and stationarity check are part of SARIMA pipeline
        ...
        if "optimisation_arima" in path_parts:
            return (2, "03_optimisation_arima")
        ...
        # Benchmark at the end of GARCH pipeline (still priority 3)
        if (
            "benchmark" in path_parts and "garch" in path_parts
        ) or "main_vol_backtest" in path_parts:
            return (3, "08_benchmark_vol_backtest")
