...
def _compute_std_resid_diagnostics(
    z: np.ndarray,
...
def _extract_direct_residuals(df_train: pd.DataFrame) -> np.ndarray:
    ...
    Raises:
        ValueError: If 'sarima_resid' column is missing or contains no valid values.

    """
    if "sarima_resid" not in df_train.columns:
        msg = "Required column 'sarima_resid' not found in training dataset."
        raise ValueError(msg)

    series_train = pd.to_numeric(df_train["sarima_resid"], errors="coerce")
    resid = np.asarray(series_train, dtype=float)
    if not np.any(np.isfinite(resid)):
        msg = "Column 'sarima_resid' contains no valid residual values."
        raise ValueError(msg)
    return resid
...
def _extract_train_residuals_with_mask(
    ...
    if not np.any(valid_mask_train):
        msg = "Column 'sarima_resid' contains no valid training residuals."
        raise ValueError(msg)
