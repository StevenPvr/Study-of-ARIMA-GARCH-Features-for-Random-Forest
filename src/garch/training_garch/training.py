...
def compute_diagnostics_with_filter(
    ...
    """
    final_params_obj = EGARCHParams.from_dict(
        final_params_dict,
        o=forecaster.o,
        p=forecaster.p,
...
def train_egarch_from_dataset(df: pd.DataFrame) -> dict[str, Any]:
    ...
    # Derive explicit window sizes from available TRAIN data.
    # Why: tests and small offline runs may have very few observations; we must
    # explicitly cap windows to what is available without silent fallbacks.
