...
def get_default_params() -> dict[str, float]:
    ...
    return {
        ...
        "nu": GARCH_STUDENT_NU_INIT,
        "lambda": GARCH_SKEWT_LAMBDA_INIT,
    }


def build_bounds_base() -> list[tuple[float, float]]:
    """Build base parameter bounds for omega, alpha, gamma, beta.

    Returns:
        List of (min, max) tuples for base parameters.
    """
    return [
        (GARCH_ESTIMATION_OMEGA_BOUND_MIN, GARCH_ESTIMATION_OMEGA_BOUND_MAX),
        (GARCH_ESTIMATION_ALPHA_BOUND_MIN, GARCH_ESTIMATION_ALPHA_BOUND_MAX),
        (GARCH_ESTIMATION_GAMMA_BOUND_MIN, GARCH_ESTIMATION_GAMMA_BOUND_MAX),
        (GARCH_ESTIMATION_BETA_MIN, GARCH_ESTIMATION_BETA_MAX),
    ]
...
def extract_params_from_array(params: np.ndarray, o: int, p: int, dist: str) -> tuple[
    ...
    egarch_params = EGARCHParams.from_array(params, o, p, dist)
    return (
        egarch_params.omega,
        egarch_params.alpha,
        egarch_params.gamma,
...
def build_result_dict(
    ...
    egarch_params = EGARCHParams.from_array(params, o, p, dist, loglik=loglik, converged=converged)
