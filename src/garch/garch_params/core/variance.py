...
This module provides variance computation for EGARCH models:
- Initial variance computation
- Variance step computation
- Full variance path computation
- EGARCH(1,1) and EGARCH(o,p) support
...
from src.constants import (
    GARCH_INIT_VAR_ESTIMATION_MIN_SIZE,
    GARCH_LOG_VAR_MAX,
    GARCH_LOG_VAR_MIN,
    GARCH_MIN_INIT_VAR,
)
...
def initialize_variance(residuals: np.ndarray, init: float | None) -> float:
    ...
    Uses provided initial variance or sample variance computed from early residuals only.

    ANTI-LEAKAGE: To preserve temporal causality, when init is None, we compute
    the initial variance using only the first min(GARCH_INIT_VAR_ESTIMATION_MIN_SIZE,
    len(residuals)) residuals. This ensures that modifying residuals at position t does not
    affect the
    initialization, which is critical for causal variance computation.
    ...
    if init is not None and init > 0:
        return float(init)
    # ANTI-LEAKAGE: Use only early residuals for initialization to preserve causality
    # Using first GARCH_INIT_VAR_ESTIMATION_MIN_SIZE residuals (30 by default, increased from 10)
    # ensures robust variance estimation while preserving causality
    n_early = min(GARCH_INIT_VAR_ESTIMATION_MIN_SIZE, len(residuals))
    if n_early > 0:
        v = float(np.var(residuals[:n_early]))
    else:
        v = GARCH_MIN_INIT_VAR
...
def _compute_recursion_step_general(
    ...
    Returns:
        Next variance value, or None if invalid.
    """
    log_variance = omega
...
def _compute_variance_path_core(
    ...
    for i in range(1, max_lag + 1):
        variances[i] = initialize_variance(residuals_arr, init)

    # Compute recursion from max_lag onwards
    start_t = max(o, p)
...
def compute_variance_path_garch11(
    residuals: np.ndarray,
    omega: float,
    alpha: float,
    beta: float,
    *,
    ...
    """Compute standard GARCH(1,1) variance path (filtered variance).

    Implements Bollerslev (1986) GARCH(1,1) model:
    σ²ₜ = ω + α·ε²ₜ₋₁ + β·σ²ₜ₋₁

    ⚠️  TEMPORAL CAUSALITY: σ²ₜ uses residuals up to t-1 (causal).
    ...
    Args:
        ...
        init: Initial variance σ²₀. If None, uses sample variance.

    Returns:
        Variance path [σ²₁, σ²₂, ..., σ²ₜ] (filtered variance).
        Returns NaN array if numerical issues occur.
    ...
    # Recursion: σ²ₜ = ω + α·ε²ₜ₋₁ + β·σ²ₜ₋₁
    for t in range(1, n):
        variances[t] = omega + alpha * (residuals_arr[t - 1] ** 2) + beta * variances[t - 1]

        # Numerical safety checks
        if not np.isfinite(variances[t]) or variances[t] <= 0:
            logger.warning(
                "GARCH(1,1) variance became non-positive or non-finite at t=%d: σ²=%g. "
                "This may indicate parameter instability (α+β≥1) or extreme residuals.",
                t,
                variances[t],
            )
            return np.full(n, np.nan)
