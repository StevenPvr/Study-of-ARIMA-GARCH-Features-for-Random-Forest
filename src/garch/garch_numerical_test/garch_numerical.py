...
def breusch_pagan_test(
    ...
    Method:
    1. Compute squared residuals e²
    2. Regress e² on exogenous variables (or time trend)
    3. Compute LM statistic = n*R² ~ χ²(k) where k = number of regressors
    ...
    # If no exogenous variables, use time trend polynomial
    if exog is None:
        t = np.arange(n, dtype=float)
        # Create polynomial trend: [1, t, t²]
        poly_order = GARCH_NUMERICAL_BREUSCH_PAGAN_POLYNOMIAL_ORDER
        exog = np.column_stack([np.ones(n)] + [t**i for i in range(1, poly_order + 1)])
    else:
    ...
    # Check dimensions
    if exog.shape[0] != n:
        msg = f"Exogenous variables must have {n} rows, got {exog.shape[0]}"
        raise ValueError(msg)
...
def white_test(
    ...
    # Create regressors: time trend polynomial up to 4th order
    t = np.arange(n, dtype=float)
    t_normalized = (t - np.mean(t)) / (np.std(t) + GARCH_NUMERICAL_WHITE_NORMALIZATION_EPSILON)

    # Create polynomial features: [1, t, t², t³, t⁴]
    poly_order = GARCH_NUMERICAL_WHITE_POLYNOMIAL_ORDER
    exog = np.column_stack([np.ones(n)] + [t_normalized**i for i in range(1, poly_order + 1)])
