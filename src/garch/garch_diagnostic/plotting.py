...
from src.constants import (
    GARCH_DIAGNOSTICS_FIGURE_SIZE,
    GARCH_DIAGNOSTICS_PLOT_ALPHA,
    GARCH_DIAGNOSTICS_PLOT_BAR_WIDTH,
    GARCH_DIAGNOSTICS_PLOT_COLOR_ACF,
    GARCH_DIAGNOSTICS_PLOT_COLOR_CONFIDENCE,
    GARCH_DIAGNOSTICS_PLOT_COLOR_GRAY,
    GARCH_DIAGNOSTICS_PLOT_COLOR_PACF,
    GARCH_DIAGNOSTICS_PLOT_COLOR_TEST,
    GARCH_DIAGNOSTICS_PLOT_COLOR_TEST_STD,
    GARCH_DIAGNOSTICS_PLOT_COLOR_TRAIN,
    GARCH_DIAGNOSTICS_PLOT_COLOR_TRAIN_STD,
    GARCH_DIAGNOSTICS_PLOT_COLOR_ZERO_LINE,
    GARCH_DIAGNOSTICS_PLOT_LEGEND_LOC,
    GARCH_DIAGNOSTICS_PLOT_LINESTYLE_DASHED,
    GARCH_DIAGNOSTICS_PLOT_LINESTYLE_DOTTED,
    GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
    GARCH_DIAGNOSTICS_PLOT_QQ_PROB_OFFSET,
    GARCH_DIAGNOSTICS_PLOT_SCATTER_SIZE,
    GARCH_DIAGNOSTICS_PLOT_STD_ERROR_DENOMINATOR,
    GARCH_PLOT_Z_CONF,
)
from src.garch.garch_diagnostic.autocorrelation import autocorr, pacf_from_autocorr
...
def _create_diagnostic_figure_canvas(
    figsize: tuple[float, float] = GARCH_DIAGNOSTICS_FIGURE_SIZE,
) -> tuple[Any, Any]:
...
def _plot_correlation_bars(
    ...
    ax.bar(
        ...
        color=color,
        width=GARCH_DIAGNOSTICS_PLOT_BAR_WIDTH,
    )
    ax.axhline(
        GARCH_PLOT_Z_CONF * se,
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_CONFIDENCE,
        linestyle=GARCH_DIAGNOSTICS_PLOT_LINESTYLE_DASHED,
        linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
    )
    ax.axhline(
        -GARCH_PLOT_Z_CONF * se,
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_CONFIDENCE,
...
def plot_acf_subplot(ax: Any, acf: np.ndarray, se: float, title: str, lags: int) -> None:
    ...
    """
    _plot_correlation_bars(ax, acf, se, title, lags, GARCH_DIAGNOSTICS_PLOT_COLOR_ACF, "ACF")
...
def plot_pacf_subplot(ax: Any, pacf: np.ndarray, se: float, title: str, lags: int) -> None:
    ...
    Args:
        ...
        lags: Number of lags to plot.
    """
    _plot_correlation_bars(ax, pacf, se, title, lags, GARCH_DIAGNOSTICS_PLOT_COLOR_PACF, "PACF")
...
def compute_acf_pacf_data(series: np.ndarray, lags: int) -> tuple[np.ndarray, np.ndarray, float]:
    ...
    se = GARCH_DIAGNOSTICS_PLOT_STD_ERROR_DENOMINATOR / np.sqrt(
        max(GARCH_DIAGNOSTICS_PLOT_STD_ERROR_DENOMINATOR, float(n))
    )
...
def compute_qq_data(
    ...
    z_sorted = np.sort(z)
    n = len(z_sorted)
    probs = (np.arange(1, n + 1) - GARCH_DIAGNOSTICS_PLOT_QQ_PROB_OFFSET) / n
...
def plot_qq_scatter(ax: Any, theo_q: np.ndarray, z_sorted: np.ndarray, title: str) -> None:
    ...
    ax.scatter(
        theo_q,
        z_sorted,
        s=GARCH_DIAGNOSTICS_PLOT_SCATTER_SIZE,
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_ACF,
        alpha=GARCH_DIAGNOSTICS_PLOT_ALPHA,
    )
...
def plot_raw_residuals(
    ...
    ax.plot(
        ...
        train,
        label="train",
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_TRAIN,
        linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
    )
    if test.size:
        ...
        ax.plot(
            ...
            test,
            label="test",
            color=GARCH_DIAGNOSTICS_PLOT_COLOR_TEST,
            linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
        )
        ax.axvline(
            n_train - GARCH_DIAGNOSTICS_PLOT_QQ_PROB_OFFSET,
            color=GARCH_DIAGNOSTICS_PLOT_COLOR_GRAY,
            linestyle=GARCH_DIAGNOSTICS_PLOT_LINESTYLE_DOTTED,
            linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
        )
    ax.set_title("Residuals (train/test)")
    ax.set_xlabel("t")
    ax.set_ylabel("e_t")
    ax.legend(loc=GARCH_DIAGNOSTICS_PLOT_LEGEND_LOC)
...
def plot_standardized_residuals(
    ...
    ax.plot(
        ...
        z_train,
        label="train",
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_TRAIN_STD,
        linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
    )
    if z_test is not None and n_test > 0:
        ax.plot(
            ...
            z_test,
            label="test",
            color=GARCH_DIAGNOSTICS_PLOT_COLOR_TEST_STD,
            linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
        )
        ax.axvline(
            n_train - GARCH_DIAGNOSTICS_PLOT_QQ_PROB_OFFSET,
    ...
    add_zero_line(
        ax,
        color=GARCH_DIAGNOSTICS_PLOT_COLOR_ZERO_LINE,
        linewidth=GARCH_DIAGNOSTICS_PLOT_LINEWIDTH,
    )
...
def create_residual_plots_figure(
    ...
    fig, canvas = _create_diagnostic_figure_canvas(figsize=GARCH_DIAGNOSTICS_FIGURE_SIZE)
