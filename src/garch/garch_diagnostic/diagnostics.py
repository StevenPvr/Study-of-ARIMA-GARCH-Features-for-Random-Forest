...
def _save_acf_pacf_plots(
    ...
    *,
    ...
    if squared:
        y = z**2 - np.mean(z**2)
        plot_name = "garch_std_squared_acf_pacf"
    else:
        y = z - np.mean(z)
        plot_name = "garch_std_acf_pacf"

    acf, pacf_vals, se = compute_acf_pacf_data(y, lags)
    out_path = prepare_output_path(outdir, filename, plot_name)

    try:
        from src.constants import GARCH_DIAGNOSTICS_FIGURE_SIZE
...
def save_qq_plot_std_residuals(
    ...
    *,
    ...
    try:
        from src.constants import GARCH_DIAGNOSTICS_QQ_FIGURE_SIZE

        fig, canvas = _create_diagnostic_figure_canvas(figsize=GARCH_DIAGNOSTICS_QQ_FIGURE_SIZE)
        ax = fig.subplots(1, 1)
