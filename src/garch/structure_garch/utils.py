...
from src.constants import (
    ...
    GARCH_PLOT_Z_CONF,
    GARCH_STRUCTURE_FIGURE_SIZE,
    GARCH_STRUCTURE_PLOT,
    GARCH_STRUCTURE_PLOT_BAR_WIDTH,
    GARCH_STRUCTURE_PLOT_COLOR_ACF,
    GARCH_STRUCTURE_PLOT_COLOR_CONFIDENCE,
    GARCH_STRUCTURE_PLOT_COLOR_RESIDUALS,
    GARCH_STRUCTURE_PLOT_COLOR_ZERO_LINE,
    GARCH_STRUCTURE_PLOT_FONTSIZE_TITLE,
    GARCH_STRUCTURE_PLOT_LINEWIDTH,
)
...
def _find_residual_column(data: pd.DataFrame) -> str:
    ...
    Returns:
        Column name 'sarima_resid'.

    Raises:
        ValueError: If 'sarima_resid' column is not present.

    """
    if "sarima_resid" not in data.columns:
        msg = "Required column 'sarima_resid' not found in dataset."
...
def _compute_residuals_from_fitted(data: pd.DataFrame) -> np.ndarray | None:
    ...
    """
    if "sarima_fitted_in_sample" not in data.columns:
        return None
    if "weighted_log_return" not in data.columns:
        return None

    returns_series = pd.to_numeric(data["weighted_log_return"], errors="coerce")
    fitted_series = pd.to_numeric(data["sarima_fitted_in_sample"], errors="coerce")
    ...
    if bool(np.any(np.isfinite(computed_residuals))):
        logger.info("Computed train residuals from weighted_log_return - sarima_fitted_in_sample")
...
def prepare_residuals(df: pd.DataFrame, use_test_only: bool = True) -> np.ndarray:
    ...
    This step enforces the presence of the column 'sarima_resid'
    ...
    Raises:
        ValueError: When 'sarima_resid' is not present or contains no valid values.
    ...
    residuals = _extract_residuals_from_column(data, col_name)
    return residuals
...
def _create_figure_and_axes(*, Figure: Any, FigureCanvas: Any) -> tuple[Any, Any, Any, Any]:
    ...
    """
    fig = Figure(figsize=GARCH_STRUCTURE_FIGURE_SIZE, constrained_layout=True)
...
def _plot_residuals_series(ax: Any, residuals: np.ndarray) -> None:
    ...
    ax.plot(
        residuals,
        color=GARCH_STRUCTURE_PLOT_COLOR_RESIDUALS,
        linewidth=GARCH_STRUCTURE_PLOT_LINEWIDTH,
    )
...
def _plot_acf_squared(ax: Any, acf_sq: np.ndarray, conf: float, acf_lags: int) -> None:
    ...
    ax.bar(
        lags,
        acf_sq,
        color=GARCH_STRUCTURE_PLOT_COLOR_ACF,
        width=GARCH_STRUCTURE_PLOT_BAR_WIDTH,
    )
    ax.axhline(
        0.0,
        color=GARCH_STRUCTURE_PLOT_COLOR_ZERO_LINE,
        linewidth=GARCH_STRUCTURE_PLOT_LINEWIDTH,
    )
    ax.axhline(
        conf,
        color=GARCH_STRUCTURE_PLOT_COLOR_CONFIDENCE,
        linestyle="--",
        linewidth=GARCH_STRUCTURE_PLOT_LINEWIDTH,
    )
    ax.axhline(
...
def render_with_matplotlib(
    *,
    ...
    fig.suptitle(
        "ARCH/GARCH identification structure", fontsize=GARCH_STRUCTURE_PLOT_FONTSIZE_TITLE
    )
