...
def forecast_on_test_from_trained_model(
    ...
    df_test["RV"] = df_test["sarima_resid"] ** 2
...
def _compute_standardized_residuals(df_garch: pd.DataFrame) -> pd.DataFrame:
    ...
    Args:
        df_garch: DataFrame with GARCH forecasts and SARIMA residuals.
    ...
    df_garch["garch_std_resid"] = np.where(
        ...
        df_garch["sarima_resid"] / np.sqrt(df_garch["garch_forecast_h1"]),
        np.nan,
    )
    return df_garch
...
def _remove_missing_values_per_ticker(df: pd.DataFrame) -> pd.DataFrame:
    ...
    subset_cols = ["garch_forecast_h1", "sarima_resid"]
...
def generate_data_tickers_full_insights(
    ...
    - ARIMA insights from rolling_predictions.csv → sarima_pred, sarima_resid
    ...
    if not arima_file.exists():
        ...
        raise FileNotFoundError(msg)
    df_arima = pd.read_csv(arima_file, parse_dates=["date"])  # date,y_true,y_pred,sarima_resid
    required_arima = {"date", "y_pred", "sarima_resid"}
    ...
    df_arima = df_arima[["date", "sarima_pred", "sarima_resid"]].copy()
    ...
    )  # date,split,weighted_log_return,sarima_resid,garch_forecast_h1,garch_vol_h1,
