...
def _map_model_name_to_distribution(model_name: str) -> str:
    ...
    Returns:
    -------
        Distribution type ('skewt', 'student', or 'normal').
...
def prepare_residuals_from_dataset(
    ...
    # sarima_resid is the required format in dataset_garch.csv
    if "sarima_resid" not in df_sorted.columns:
        msg = "Required ARIMA residuals column 'sarima_resid' not found in dataset."
        raise ValueError(msg)
    series = pd.to_numeric(df_sorted["sarima_resid"], errors="coerce")
    resid = np.asarray(series, dtype=float)
    valid_mask = np.isfinite(resid)

    if not np.any(valid_mask):
        msg = (
            "No valid residuals found in column 'sarima_resid'. "
