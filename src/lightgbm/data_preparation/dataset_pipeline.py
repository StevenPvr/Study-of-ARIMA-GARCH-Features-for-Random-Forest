...
def drop_rows_with_missing_features(df: pd.DataFrame, keep_cols: list[str]) -> pd.DataFrame:
    ...
    df_filtered = cast(pd.DataFrame, df.loc[:, keep_cols].copy())

    if not subset:
        return cast(pd.DataFrame, df_filtered.reset_index(drop=True))

    return cast(pd.DataFrame, df_filtered.dropna(subset=subset).reset_index(drop=True))
