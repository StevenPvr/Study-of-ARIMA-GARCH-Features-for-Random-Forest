...
def add_split_column(df: pd.DataFrame) -> pd.DataFrame:
    """Add split column if missing using a temporal 80/20 split by date.

    Sorts by `date` to ensure chronological order before assigning the
    split labels. This avoids any accidental leakage due to unsorted input.

    Args:
        df: DataFrame expected to contain a `date` column.

    Returns:
        DataFrame with a `split` column ('train'/'test').
    """
    if "split" in df.columns:
        return df

    if "date" not in df.columns:
        logger.warning("No 'date' column found; applying index-based 80/20 split.")
        from src.constants import LIGHTGBM_TRAIN_TEST_SPLIT_RATIO

        split_idx = int(len(df) * LIGHTGBM_TRAIN_TEST_SPLIT_RATIO)
        df_out = df.copy()
        df_out["split"] = ["train"] * split_idx + ["test"] * (len(df) - split_idx)
        return df_out

    logger.info("Adding temporal split column (80% train, 20% test) sorted by date")
    df_sorted = df.sort_values("date").reset_index(drop=True).copy()
    from src.constants import LIGHTGBM_TRAIN_TEST_SPLIT_RATIO

    split_idx = int(len(df_sorted) * LIGHTGBM_TRAIN_TEST_SPLIT_RATIO)
    df_sorted["split"] = ["train"] * split_idx + ["test"] * (len(df_sorted) - split_idx)
    return df_sorted
