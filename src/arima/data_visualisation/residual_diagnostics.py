...
from src.constants import (
    ...
    RESIDUALS_HISTOGRAM_BINS_DEFAULT,
...
def _plot_residuals_timeseries_panel(
    ...
    plot_series_with_train_test_split(
        ...
        train_color=PLOT_COLOR_TRAIN,
        test_color=PLOT_COLOR_TEST,
    ...
    ax.set_ylabel("Residual", fontsize=PLOT_FONTSIZE_LABEL)
...
def _plot_residuals_histogram_panel(
    ...
    ax.set_xlabel("Residual", fontsize=PLOT_FONTSIZE_LABEL)
...
def _plot_residuals_qq_panel(ax: Axes, residuals: pd.Series) -> float:
    ...
    ax.set_xlabel("Quantiles théoriques (Normal)", fontsize=PLOT_FONTSIZE_LABEL)
    ax.set_ylabel("Quantiles de l'échantillon", fontsize=PLOT_FONTSIZE_LABEL)
...
def _plot_residuals_acf_panel(
    ...
    ax.set_xlabel("Lag", fontsize=PLOT_FONTSIZE_LABEL)
    ax.set_ylabel("Autocorrelation", fontsize=PLOT_FONTSIZE_LABEL)
...
def plot_residuals_timeseries(
    ...
    residuals_series = df["residual"].dropna()

    _, ax = create_standard_figure(figsize=(14, 5))
...
def plot_residuals_histogram(
    ...
    bins: int = RESIDUALS_HISTOGRAM_BINS_DEFAULT,
) -> None:
...
def plot_comprehensive_residuals(
    ...
    if "residual" not in df.columns:
        msg = "Column 'residual' not found in predictions file"
        raise ValueError(msg)

    residuals = df["residual"].dropna()

    if residuals.empty:
        msg = "No valid residuals found"
        raise ValueError(msg)
    ...
    fig.suptitle(
        ...
        fontsize=PLOT_FONTSIZE_TITLE + 2,
        fontweight="bold",
        y=0.995,
    )
