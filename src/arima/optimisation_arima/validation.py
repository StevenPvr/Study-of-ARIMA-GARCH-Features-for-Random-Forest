"""Validation helpers for SARIMA optimization."""

from __future__ import annotations

import pandas as pd

from src.constants import SARIMA_BACKTEST_MIN_TRAIN_MARGIN
...
def validate_series(name: str, s: pd.Series) -> None:
    """Validate that a pandas Series meets requirements for SARIMA modeling.
...
def _validate_parameter_types_and_ranges(
    ...
    """Validate that all SARIMA parameters are non-negative integers."""
    param_names = ("p", "d", "q", "P", "D", "Q", "s")
    param_values = (p, d, q, P, D, Q, s)
...
def _validate_seasonal_consistency(P: int, D: int, Q: int, s: int) -> None:
    """Validate seasonal parameter consistency."""
    seasonal_orders = (P, D, Q)

    if s == 0 and any(order > 0 for order in seasonal_orders):
        raise ValueError("Seasonal orders must be zero when s=0.")

    if s == 1 and any(order > 0 for order in seasonal_orders):
        raise ValueError("Seasonal orders must be zero when s=1.")


def validate_sarima_params(p: int, d: int, q: int, P: int, D: int, Q: int, s: int) -> None:
    """Validate SARIMA parameter values.
    ...
    Raises:
        ValueError: If any parameter is negative or seasonal orders are non-zero when s=0 or s=1.
    """
    _validate_parameter_types_and_ranges(p, d, q, P, D, Q, s)
    _validate_seasonal_consistency(P, D, Q, s)
...
def validate_backtest_config(
    ...
    min_margin = max(SARIMA_BACKTEST_MIN_TRAIN_MARGIN, test_size)
